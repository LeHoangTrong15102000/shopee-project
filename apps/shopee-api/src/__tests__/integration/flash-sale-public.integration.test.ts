...
describe('Public Flash Sale Endpoints (Task 10.5)', () => {
  ...
  describe('GET /flash-sales/:id/products', () => {
    it('returns products with stock info for ACTIVE sale', async () => {
      ...
      expect(res.status).toBe(200)
      expect(res.body.data.length).toBeGreaterThanOrEqual(1)
      expect(res.body.data[0]).toHaveProperty('flashPrice')
      expect(res.body.data[0]).toHaveProperty('totalQuantity')
      expect(res.body.data[0]).toHaveProperty('soldQuantity')
      expect(res.body.data[0]).toHaveProperty('remainingQuantity')
    })
  })
})
