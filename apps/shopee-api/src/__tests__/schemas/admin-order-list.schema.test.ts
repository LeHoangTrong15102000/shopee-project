...
describe('Admin Order List Schemas', () => {
  describe('adminOrderListSchema', () => {
    it('should accept valid query', () => {
      expect(
        adminOrderListSchema.safeParse({ query: { status: 'pending', page: 1 } }).success,
      ).toBe(true)
    })
    it('should accept payment_method filter', () => {
      expect(adminOrderListSchema.safeParse({ query: { payment_method: 'cod' } }).success).toBe(
        true,
      )
    })
    ...
    it('should reject invalid payment_method', () => {
      expect(adminOrderListSchema.safeParse({ query: { payment_method: 'bitcoin' } }).success).toBe(
        false,
      )
    })
  })
  ...
  describe('adminBulkUpdateStatusSchema', () => {
    it('should accept valid input', () => {
      expect(
        adminBulkUpdateStatusSchema.safeParse({
