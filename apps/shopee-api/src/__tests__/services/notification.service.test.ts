...
describe('NotificationService', () => {
  ...
  describe('getNotifications', () => {
    it('should return paginated results', async () => {
      const mockResult = { data: [{ id: notificationId, title: 'Test' }], pagination: { page: 1, limit: 10, page_size: 1, total: 1 } } as any
      ...
      expect(mockNotificationRepository.findByUser).toHaveBeenCalledWith(userId, {}, { page: 1, limit: 10 })
  ...
  describe('markAsRead', () => {
    it('should mark notification as read when found', async () => {
      const mockNotification = { _id: new Types.ObjectId(), user: new Types.ObjectId(userId), title: 'Test', content: 'Content', type: 'other', is_read: true } as any
  ...
  describe('createNotification', () => {
    it('should create notification with default type other', async () => {
      const mockNotification = { _id: new Types.ObjectId(), user: new Types.ObjectId(userId), title: 'Test', content: 'Content', type: 'other', is_read: false } as any
    ...
    it('should create notification with custom type and link', async () => {
      const mockNotification = { _id: new Types.ObjectId(), user: new Types.ObjectId(userId), title: 'Test', content: 'Content', type: 'order', is_read: false, link: '/orders/1' } as any
      mockNotificationRepository.createNotification.mockResolvedValue(mockNotification)

      const result = await service.createNotification(userId, 'Test', 'Content', 'order' as any, '/orders/1')

      expect(mockNotificationRepository.createNotification).toHaveBeenCalledWith(
        expect.objectContaining({ title: 'Test', content: 'Content', type: 'order', link: '/orders/1' })
      )
      expect(result).toEqual(mockNotification)
    })
  })
