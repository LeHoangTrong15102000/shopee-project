...
const vouchersApi = {
  ...
  getVoucher: (id: string) =>
    http.get<SuccessResponse<Voucher>>(`admin/vouchers/${id}`),

  getVoucherUsage: (id: string, params?: { page?: number; limit?: number }) =>
    http.get<SuccessResponse<{ usage: VoucherUsage[]; pagination: { page: number; limit: number; total: number } }>>(`admin/vouchers/${id}/usage`, { params }),

  getVoucherStats: () =>
    http.get<SuccessResponse<VoucherStats>>('admin/vouchers/stats'),
  ...
  deleteVoucher: (id: string) =>
    http.delete<SuccessResponse<null>>(`admin/vouchers/${id}`),
}
