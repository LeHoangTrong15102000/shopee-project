...
export default function ImportPage() {
  ...
  return (
    <div className="space-y-6">
      ...
      <Card>
        <CardHeader><CardTitle>Import Products</CardTitle></CardHeader>
        <CardContent className="space-y-4">
          <p className="text-sm text-muted-foreground">
            This will import products from the server-side JSON file. All existing products will be replaced.
          </p>
          <Button onClick={() => setConfirmOpen(true)} disabled={importMut.isPending}>
            {importMut.isPending ? <Loader2 className="mr-2 size-4 animate-spin" /> : <Upload className="mr-2 size-4" />}
            Import Products
          </Button>
          {importMut.data && (
