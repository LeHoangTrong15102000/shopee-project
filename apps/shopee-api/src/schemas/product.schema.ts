...
const variantOptionSchema = z.object({
  name: z
    ...
    .refine((val) => val.trim().length > 0, {
      message: 'Tên tùy chọn không được chỉ chứa khoảng trắng',
    }),
  value: z
    .string()
    ...
    .refine((val) => val.trim().length > 0, {
      message: 'Giá trị tùy chọn không được chỉ chứa khoảng trắng',
    }),
...
const variantItemSchema = z.object({
  type: z.enum(VARIANT_TYPES, {
    message: 'Loại biến thể phải là: color, size, style, hoặc material',
  }),
  name: z
    .string()
    .min(1, 'Tên biến thể không được để trống')
    .max(100, 'Tên biến thể phải ít hơn 100 ký tự'),
 ...
 */
const variantsArraySchema = z.array(variantItemSchema).superRefine((variants, ctx) => {
...
export const addProductSchema = z.object({
  body: z
    .object({
      ...
      price: z.coerce.number().min(0, 'Giá sản phẩm phải lớn hơn hoặc bằng 0'),
      quantity: z.coerce
        .number()
      ...
      images: z.array(z.string().max(1000, 'Mỗi URL ảnh phải ít hơn 1000 ký tự')).optional(),
      description: z.string().max(10000, 'Mô tả phải ít hơn 10000 ký tự').optional(),
...
export const updateProductSchema = z.object({
  body: z
    .object({
      name: z.string().max(160, 'Tên sản phẩm phải ít hơn 160 ký tự').optional(),
      price: z.coerce.number().min(0, 'Giá sản phẩm phải lớn hơn hoặc bằng 0').optional(),
      quantity: z.coerce
        .number()
        .int('Số lượng phải là số nguyên không âm')
        .min(0, 'Số lượng phải là số nguyên không âm')
...
export const getProductsSchema = z.object({
  query: z
    .object({
      ...
      price_min: z.coerce.number().min(0, 'Giá tối thiểu phải lớn hơn hoặc bằng 0').optional(),
      price_max: z.coerce.number().min(0, 'Giá tối đa phải lớn hơn hoặc bằng 0').optional(),
      category: mongoIdSchema
        .refine((val) => val, { message: 'Danh mục không đúng định dạng' })
        .optional(),
...
export const getPagesSchema = z.object({
  query: z
    .object({
      limit: z.coerce.number().int('limit không đúng định dạng'),
      category: mongoIdSchema
        .refine((val) => val, { message: 'category không đúng định dạng' })
        .optional(),
    })
