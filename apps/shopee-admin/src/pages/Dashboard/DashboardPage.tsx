...
import { Area, AreaChart, Bar, BarChart, CartesianGrid, Cell, Line, LineChart, Pie, PieChart, XAxis, YAxis, Tooltip, ResponsiveContainer } from 'recharts'
...
export default function DashboardPage() {
  ...
  return (
    <div className="space-y-6">
      <PageHeader title="Dashboard" description="Overview of your store performance" actions={<PeriodSelect value={period} onChange={setPeriod} />} />

      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        <StatCard label="Total Revenue" value={overview?.total_revenue ?? 0} trend={overview?.revenue_change} formatter={formatCurrency} icon={<DollarSign className="size-4" />} />
        <StatCard label="Total Orders" value={overview?.total_orders ?? 0} trend={overview?.orders_change} icon={<ShoppingCart className="size-4" />} />
        <StatCard label="Total Users" value={overview?.total_users ?? 0} trend={overview?.users_change} icon={<Users className="size-4" />} />
        <StatCard label="Total Products" value={overview?.total_products ?? 0} trend={overview?.products_change} icon={<Package className="size-4" />} />
      </div>
      ...
      <div className="grid gap-4 lg:grid-cols-2">
        ...
        <Card>
          ...
          <CardContent>
            <ChartContainer config={{ revenue: { label: 'Revenue', color: 'var(--chart-1)' } }} className="h-[300px]">
              <AreaChart data={revenue ?? []}>
                <CartesianGrid strokeDasharray="3 3" />
                <XAxis dataKey="date" fontSize={12} tickLine={false} />
                <YAxis fontSize={12} tickLine={false} tickFormatter={(v) => `$${v}`} />
                <ChartTooltip content={<ChartTooltipContent />} />
                <Area type="monotone" dataKey="revenue" fill="var(--color-revenue)" fillOpacity={0.3} stroke="var(--color-revenue)" />
              </AreaChart>
            </ChartContainer>
          </CardContent>
        </Card>
        ...
        <Card>
          ...
          <CardContent>
            <ChartContainer config={{ orders: { label: 'Orders', color: 'var(--chart-2)' } }} className="h-[300px]">
      ...
      <div className="grid gap-4 lg:grid-cols-3">
        ...
        <Card>
          ...
          <CardContent>
            <ChartContainer config={{ users: { label: 'Users', color: 'var(--chart-3)' } }} className="h-[250px]">
        ...
        <Card className="lg:col-span-2">
          ...
          <CardContent className="flex items-center justify-center">
            <ResponsiveContainer width="100%" height={250}>
              <PieChart>
                <Pie data={revenueByCategory ?? []} dataKey="revenue" nameKey="category" cx="50%" cy="50%" outerRadius={90} label={({ category, percent }) => `${category} ${(percent * 100).toFixed(0)}%`}>
