...
import {
  adminRewardListSchema, adminCreateRewardSchema, adminUpdateRewardSchema,
  adminRewardIdSchema, adminAdjustPointsSchema, adminTransactionListSchema,
...
describe('Admin Loyalty Schemas', () => {
  describe('adminRewardListSchema', () => {
    it('should accept valid query', () => {
      expect(adminRewardListSchema.safeParse({ query: { reward_type: 'voucher' } }).success).toBe(true)
    })
    it('should reject invalid reward_type', () => {
      expect(adminRewardListSchema.safeParse({ query: { reward_type: 'invalid' } }).success).toBe(false)
    })
  })

  describe('adminCreateRewardSchema', () => {
    const validBody = {
      name: 'Test Reward', points_required: 100, reward_type: 'voucher' as const,
      reward_value: 50, stock: 10,
    }
    it('should accept valid input', () => {
      expect(adminCreateRewardSchema.safeParse({ body: validBody }).success).toBe(true)
    })
    it('should reject empty name', () => {
      expect(adminCreateRewardSchema.safeParse({ body: { ...validBody, name: '' } }).success).toBe(false)
    })
    it('should reject negative points', () => {
      expect(adminCreateRewardSchema.safeParse({ body: { ...validBody, points_required: -1 } }).success).toBe(false)
    })
  })

  describe('adminUpdateRewardSchema', () => {
    it('should accept partial update', () => {
      expect(adminUpdateRewardSchema.safeParse({ params: { id: VALID_ID }, body: { stock: 5 } }).success).toBe(true)
    })
  })
  ...
  describe('adminAdjustPointsSchema', () => {
    it('should accept valid adjustment', () => {
      expect(adminAdjustPointsSchema.safeParse({
