...
describe('useDeleteQuestion', () => {
  it('deletes a question', async () => {
    ...
    const { result } = renderHook(() => useDeleteQuestion(onSuccess), { wrapper: createQueryWrapper() });
...
describe('useDeleteAnswer', () => {
  it('deletes an answer', async () => {
    const onSuccess = vi.fn();
    const { result } = renderHook(() => useDeleteAnswer(onSuccess), { wrapper: createQueryWrapper() });
