...
const mockWishlistItems: WishlistItem[] = [
  ...
  _mp(
    2,
    'Tai nghe Bluetooth không dây chống ồn ANC',
    450000,
    650000,
    'c2',
    'Điện tử',
  ...
  _mp(
    4,
    'Váy liền thân nữ phong cách Hàn Quốc',
    189000,
    299000,
    'c4',
    'Thời trang nữ',
  ...
  _mp(
    10,
    'Ốp lưng iPhone 15 Pro Max silicon',
    59000,
    120000,
    'c3',
    'Phụ kiện',
  ...
  _mp(
    12,
    'Loa Bluetooth mini di động chống nước',
    350000,
    550000,
    'c2',
    'Điện tử',
  ...
  _mp(
    15,
    'Áo khoác gió nữ 2 lớp chống UV',
    345000,
    520000,
    'c4',
    'Thời trang nữ',
...
const wishlistApi = {
  ...
  checkInWishlist: async (productId: string, options?: ApiOptions) => {
    try {
      const response = await http.get<SuccessResponseApi<WishlistCheckResponse>>(
  ...
  getWishlistCount: async (options?: ApiOptions) => {
    try {
      const response = await http.get<SuccessResponseApi<WishlistCountResponse>>(
        '/wishlist/count',
        {
          signal: options?.signal,
        },
