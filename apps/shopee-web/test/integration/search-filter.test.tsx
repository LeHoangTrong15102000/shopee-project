...
describe('Search and Filter Integration Tests', () => {
  ...
  test('Guest user can see search input on homepage', { timeout: 10000 }, async () => {
  ...
  test('Search input accepts text input', { timeout: 10000 }, async () => {
  ...
  test('Page loads with product listing', { timeout: 10000 }, async () => {
    renderWithRouter({ route: path.home })
  ...
  test('URL updates when navigating to products page', { timeout: 10000 }, async () => {
    renderWithRouter({ route: path.products })

    await waitFor(
      () => {
        expect(
          window.location.pathname === path.products || window.location.pathname === '/',
        ).toBeTruthy()
      },
      { timeout: 3000 },
    )
  ...
  test('Filter by category navigates correctly', { timeout: 10000 }, async () => {
    const { user } = renderWithRouter({ route: path.home })
    ...
    if (categoryLink) {
      ...
      await waitFor(
        () => {
          expect(
            window.location.search.includes('category') || window.location.pathname.length > 0,
          ).toBeTruthy()
        },
        { timeout: 3000 },
      )
