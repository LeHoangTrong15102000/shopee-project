...
describe('SKU Schemas', () => {
  describe('createSKUSchema', () => {
    it('should accept valid input', () => {
      expect(
        createSKUSchema.safeParse({ body: { value: 'SKU-001', price: 100, stock: 50 } }).success,
      ).toBe(true)
    })
    it('should reject empty value', () => {
      expect(
        createSKUSchema.safeParse({ body: { value: '', price: 100, stock: 50 } }).success,
      ).toBe(false)
    })
    it('should reject negative price', () => {
      expect(
        createSKUSchema.safeParse({ body: { value: 'SKU-001', price: -1, stock: 50 } }).success,
      ).toBe(false)
    })
    it('should reject negative stock', () => {
      expect(
        createSKUSchema.safeParse({ body: { value: 'SKU-001', price: 100, stock: -1 } }).success,
      ).toBe(false)
    })
    it('should accept optional variant_values', () => {
      expect(
        createSKUSchema.safeParse({
