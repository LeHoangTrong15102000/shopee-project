  ...
  const renderOption = ({ item, index }: { item: SelectOption; index: number }) => {
    ...
    return (
      <Pressable
        ...
        className={cn(
          'flex-row items-center border-b border-neutrals900 px-4 py-3',
          item.disabled && 'opacity-50',
          itemSelected && 'border-primary/10 bg-primary/10',
        ...
        {multiple && (
          <View
            className={cn(
              ...
              itemSelected ? 'border-primary bg-primary' : 'border-neutrals400'
            )}>
            {itemSelected && <Icon name="Check" className="h-3 w-3 text-white" />}
          </View>
        )}
        <Text
          className={cn('flex-1 font-sans-medium text-foreground', itemSelected && 'text-primary')}>
          {item.label}
        </Text>
        ...
        {renderSelector ? (
          ...
          <>
            <Pressable
              ...
              <Icon name="ChevronDown" className="h-5 w-5 text-neutrals100" />
            </Pressable>
