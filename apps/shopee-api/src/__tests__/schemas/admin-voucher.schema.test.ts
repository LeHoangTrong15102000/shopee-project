...
describe('Admin Voucher Schemas', () => {
  describe('adminVoucherListSchema', () => {
    it('should accept valid query', () => {
      expect(
        adminVoucherListSchema.safeParse({
          query: { discount_type: 'percentage', is_active: 'true' },
        }).success,
      ).toBe(true)
    })
    it('should reject invalid discount_type', () => {
      expect(
        adminVoucherListSchema.safeParse({ query: { discount_type: 'invalid' } }).success,
      ).toBe(false)
    })
  ...
  describe('adminCreateVoucherSchema', () => {
    ...
    it('should reject short code', () => {
      expect(
        adminCreateVoucherSchema.safeParse({ body: { ...validBody, code: 'AB' } }).success,
      ).toBe(false)
    })
    it('should reject lowercase code', () => {
      expect(
        adminCreateVoucherSchema.safeParse({ body: { ...validBody, code: 'save20' } }).success,
      ).toBe(false)
    })
    it('should reject negative discount_value', () => {
      expect(
        adminCreateVoucherSchema.safeParse({ body: { ...validBody, discount_value: -1 } }).success,
      ).toBe(false)
    })
    it('should reject invalid date format', () => {
      expect(
        adminCreateVoucherSchema.safeParse({ body: { ...validBody, start_date: '01-01-2024' } })
          .success,
      ).toBe(false)
    })
  })
  ...
  describe('adminUpdateVoucherSchema', () => {
    it('should accept partial update', () => {
      expect(
        adminUpdateVoucherSchema.safeParse({
          params: { id: VALID_ID },
          body: { discount_value: 30 },
        }).success,
      ).toBe(true)
    })
  ...
  describe('adminVoucherUsageSchema', () => {
    it('should accept valid params and query', () => {
      expect(
        adminVoucherUsageSchema.safeParse({
          params: { id: VALID_ID },
