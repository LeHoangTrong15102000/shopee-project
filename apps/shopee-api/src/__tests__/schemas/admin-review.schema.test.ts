...
describe('Admin Review Schemas', () => {
  describe('adminReviewListSchema', () => {
    it('should accept valid query', () => {
      expect(adminReviewListSchema.safeParse({ query: { rating: 5, product_id: VALID_ID } }).success).toBe(true)
    })
    ...
    it('should accept search and sort', () => {
      expect(adminReviewListSchema.safeParse({ query: { search: 'good', sort_by: 'created_at', order: 'asc' } }).success).toBe(true)
    })
  })
