        ...
        return (
          <BottomSheetModal
            ...
            enablePanDownToClose={
              dialog.options.dismissable !== false && dialog.options.type !== 'loading'
            }
            ...
            <BottomSheetView
              ...
              <View className="px-6 py-4">
                {/* Icon */}
                {dialog.options.icon && (
                  <View className="mb-4 items-center">{dialog.options.icon}</View>
                )}
                ...
                {dialog.options.type === 'loading' && (
                  <View className="items-center py-6">
                    ...
                    {dialog.options.title && (
                      <Text className="mt-4 text-center font-sans-semibold text-lg text-foreground">
                        {dialog.options.title}
                      </Text>
                    )}
                    {dialog.options.message && (
                      <Text className="mt-2 text-center font-sans-regular text-sm text-neutrals300">
                        {dialog.options.message}
                      </Text>
                    )}
                  </View>
                ...
                {dialog.options.type !== 'loading' && (
                  <>
                    {/* Title */}
                    {dialog.options.title && (
                      <Text className="mb-3 text-center font-sans-bold text-xl text-foreground">
                        {dialog.options.title}
                      </Text>
                    )}
                    ...
                    {/* Message */}
                    {dialog.options.message && (
                      <Text className="mb-6 text-center font-sans-regular text-base text-neutrals300">
                        {dialog.options.message}
                      </Text>
                    )}
                    ...
                    {buttons.length > 0 && (
                      <View
                        ...
                        {buttons.map((button, index) => (
                          <View
                            key={index}
                            className={buttonLayout === 'horizontal' ? 'flex-1' : ''}>
                            <AppButton
