...
const NotificationBell = ({ isAuthenticated, unreadCount }: NotificationBellProps) => {
  ...
  return (
    ...
    >
      ...
      <span className="mx-1 hidden text-xs capitalize md:inline md:text-sm">
...
const UserMenu = ({ profile, handleLogout }: UserMenuProps) => {
  ...
  return (
    <Popover
      ...
      renderPopover={
        <div className="relative min-w-[180px] overflow-hidden rounded-lg border border-gray-200 bg-white shadow-md transition-all dark:border-slate-700 dark:bg-slate-800">
          <div
            className={classNames(
      ...
      className={classNames(
        'ml-2 flex cursor-pointer items-center py-1 hover:text-white/70 md:ml-5',
      )}
    >
