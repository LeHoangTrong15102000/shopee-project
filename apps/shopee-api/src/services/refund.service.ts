...
import { BaseService, NotFoundError, BusinessError, ConflictError, ForbiddenError } from './base.service'
import { IRefundRepository, RefundFilterOptions } from '@repositories/interfaces/refund.repository.interface'
...
export class RefundService extends BaseService {
  ...
  async createRefundRequest(
    ...
    if (order.status !== ORDER_STATUS.DELIVERED && order.status !== ORDER_STATUS.RETURNED) {
      throw new BusinessError(
        'Chỉ có thể yêu cầu hoàn tiền cho đơn hàng đã giao hoặc đã trả hàng',
      )
    }
  ...
  async approveRefund(
    ...
    await this.orderRepository.updateStatus(refund.order_id.toString(), ORDER_STATUS.REFUND_APPROVED)
