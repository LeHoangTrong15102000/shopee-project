...
describe('ShopeeCheckbox Component Unit Tests', () => {
  ...
  describe('Rendering', () => {
    ...
    test('should render with different sizes', () => {
      const { rerender, container } = renderWithTheme(
        <ShopeeCheckbox {...defaultProps} size="sm" />,
      )
    ...
    test('should render with custom className', () => {
      const { container } = renderWithTheme(
        <ShopeeCheckbox {...defaultProps} className="custom-class" />,
      )
  ...
  describe('Accessibility', () => {
    ...
    test('should have proper aria-checked attribute', () => {
      const { container, rerender } = renderWithTheme(
        <ShopeeCheckbox checked={false} onChange={vi.fn()} />,
      )
  ...
  describe('Visual States', () => {
    ...
    test('should handle different size variations correctly', () => {
      ...
      sizes.forEach((size, index) => {
        const { container } = renderWithTheme(
          <ShopeeCheckbox checked={false} onChange={vi.fn()} size={size} />,
        )
  ...
  describe('Edge Cases', () => {
    test('should handle boolean checked prop correctly', () => {
      const { rerender, container } = renderWithTheme(
        <ShopeeCheckbox checked={true} onChange={vi.fn()} />,
      )
    ...
    test('should maintain state consistency with props', () => {
      ...
      const { container } = renderWithTheme(
        <ShopeeCheckbox checked={false} onChange={mockOnChange} />,
      )
  ...
  describe('Disabled State', () => {
    test('should not call onChange when disabled and clicked', () => {
      const onChange = vi.fn()
      const { container } = renderWithTheme(
        <ShopeeCheckbox checked={false} onChange={onChange} disabled={true} />,
      )
      const checkbox = container.querySelector('[role="checkbox"]')
      fireEvent.click(checkbox!)
      expect(onChange).not.toHaveBeenCalled()
    ...
    test('should have opacity-50 class when disabled', () => {
      const { container } = renderWithTheme(
        <ShopeeCheckbox checked={false} onChange={vi.fn()} disabled={true} />,
      )
      const checkbox = container.querySelector('[role="checkbox"]')
