...
import {
  WishlistContext,
  WishlistCheckQueryData,
  WishlistCountQueryData,
  QUERY_KEYS,
} from '../shared/types';
  ...
  const addMutation = useMutation({
    ...
    onMutate: async (): Promise<WishlistContext> => {
      ...
      queryClient.setQueryData(
        QUERY_KEYS.WISHLIST_CHECK(productId),
        (old: WishlistCheckQueryData | undefined) => ({
          ...old,
      ...
      queryClient.setQueryData(
        ...
        (old: WishlistCountQueryData | undefined) => {
          if (!old) return old;
          return {
            ...old,
